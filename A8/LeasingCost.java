public class LeasingCost {
	...
	public static Vehicle createVehicle(String description){
        ...
        for(int i = 0; i < fragments.length; i++){
            String[] temp = fragments[i].split(":");
            temp[0] = temp[0].replaceAll("\\s", "");
            if(temp[0].equals("name")){
                p1 = temp[1];
            }
            else if(temp[0].equals("type")){
                if(temp[1].equals("gas")){
                    p2a = true;
                }
            }
            else if(temp[0].equals("mile/unit")){
                p4 = Double.parseDouble(temp[1]);
            }
            else if(temp[0].equals("due")){
                p6 = Double.parseDouble(temp[1]);
            }
            else if(temp[0].equals("length")){
                p7 = Integer.parseInt(temp[1]);
            }
            else if(temp[0].equals("monthly")){
                p8 = Double.parseDouble(temp[1]);
            }
            else if(temp[0].equals("allowance")){
                p9 = Integer.parseInt(temp[1]);
            }
            else if(temp[0].equals("charger")){
                p5 = Double.parseDouble(temp[1]);
            }
        }
        ...
        p3 = new Lease(p6, p7, p8, p9);

	    return new Vehicle(p1, p2, p3);
	}
	...
	public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ){
            for(Vehicle v: vehicles){
                if(v.getFuel().getType() == 1){
                    v.setCO2Emission(computeCO2(v.getLease().getLeaseLength(), v.getFuel().getUsage(), v.getLease().getMileageAllowance(), 8.887));
                    v.setFuelCost(computeFuelCost(v.getLease().getLeaseLength(), v.getFuel().getUsage(), v.getLease().getMileageAllowance(), gasPrice));
                    v.setTotalCost(v.getFuelCost() + computeLeaseCost(v.getLease().getDueAtSigning(), v.getLease().getLeaseLength(), v.getLease().getMonthlyCost()));
                }
                else{
                    v.setCO2Emission(computeCO2(v.getLease().getLeaseLength(), v.getFuel().getUsage(), v.getLease().getMileageAllowance(), 0.453));
                    ...
                    v.setTotalCost(v.getFuelCost() + computeLeaseCost(v.getLease().getDueAtSigning(), v.getLease().getLeaseLength(), v.getLease().getMonthlyCost()) + v.getFuel().getCharger());
                } 
            }
    	}
