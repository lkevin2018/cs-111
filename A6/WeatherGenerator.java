...
public class WeatherGenerator {
    ...
    public static void populateLocationProbabilities( double[] drywetProbability, double[] wetwetProbability, double longitude, double latitude, double[][] drywet, double[][] wetwet){
        ...
        for(int j = 0; j < drywetProbability.length; j++){
            drywetProbability[j] = drywet[r][j+2];
        }
        for(int k = 0; k < wetwetProbability.length; k++){
            wetwetProbability[k] = wetwet[r][k+2];
        }
    }
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
        ...
        while(i < forecast.length){
            int spell = 0;
            while(i < forecast.length && forecast[i] == mode){
                spell++;
                i++;
            }
            if(spell > max){
    ...
    public static int bestWeekToTravel(int[] forecast){
        int[] weeks = new int[forecast.length-6];
        int max = Integer.MIN_VALUE;
        int idx = -1;
        for(int i = 0; i < weeks.length; i++){
            int[] newForecast = new int[7];
            System.arraycopy(forecast, i, newForecast, 0, 7);
            for(int j = 0; j < newForecast.length; j++){
                if(newForecast[j] == DRY){
                    weeks[i] += 1;
                }
            }
        }
    ...
    public static void main (String[] args) {
        ...
        StdOut.println("There are " + forecast.length + " days in the forecast for month " + 4);
