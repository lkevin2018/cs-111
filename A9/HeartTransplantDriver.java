...
public class HeartTransplantDriver {
        ...
        StdOut.println("Patients whose heart disease was caused by accidents: ");
        Patient patientsByUrgencyEX[] = ht.getPatientsByUrgency(Patient.URGENCY_EXTREME);
        Patient patientsByUrgencyMOD[] = ht.getPatientsByUrgency(Patient.URGENCY_MODERATE);
        StdOut.println("URGENCY");
        int urgency = 0;
        int moderate = 0;

        for (Patient p : patientsByUrgencyEX) {
            StdOut.println(p.toString() );
            urgency++;
        }
        StdOut.println("moderate");
        for (Patient p : patientsByUrgencyMOD) {
            StdOut.println(p.toString() );
            moderate++;
        }
        StdOut.println("urgency: " + urgency);
        StdOut.println("moderate: " + moderate);
        StdOut.println("sum: " + (urgency + moderate));
        StdOut.println();
